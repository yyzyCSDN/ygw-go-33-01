...
type Service struct {
	...
	notifier      Notifier
	snapshotCache map[string]model.ZoneSnapshot
}
...
func New(zones *zone.Store, notifier Notifier) *Service {
	return &Service{
		...
		notifier:      notifier,
		snapshotCache: make(map[string]model.ZoneSnapshot),
	}
}
...
func (s *Service) StartTransfer(name string) (model.TransferState, error) {
	...
	s.states[name] = model.TransferInProgress
	if z, err := s.zones.Get(name); err == nil {
		snapshot, _ := z.Snapshot()
		s.snapshotCache[name] = snapshot
	}
	return model.TransferInProgress, nil
}
